...
\twocolumn[
\icmltitle{Inferring Gender from Twitter Data}
...
Social media are popular platforms which can record a variety of users' personal information. Among these information, demographic characteristics and space-time travel patterns are two of great interest to geographers. Demographic characteristics include gender, age, occupation and so on. Space-time travel patterns refer to the law of people's travel trajectories along both space and time (e.g., periodicity of visiting a restaurant, time of leaving home on weekdays). Traditionally, these information are gathered from surveys, interviews, cell phone records and so on, which either cost money and resources, or encroach privacy. Nowadays, social media data could take part of these works with little expense and free of privacy infringement. Researchers have created robust framework and methodologies to mine space-time travel patterns from the geo-tagged online publications with temporal stamps. Different travel patterns could be mined for different demographic subgroups, to study the inner structure of users’ travel patterns. However, direct demographic information (e.g., gender and occupation information on Twitter) is missing for some online platforms, which makes travel trajectory studies using these social media incomplete.

It is important to restore user demographic information to link them with space-time travel patterns to unveil their relationships. The results could indicate social segmentation and contribute to public facility development for different subgroups. Up to now, there are tremendous studies on either space-time travel patterns or demographic identifications using social media data. However, few studies are conducted to investigate their relationships. Thus the travel patterns are discussed on a general group of people, which is insufficient for human mobility studies as diverse travel patterns of different subgroups have long been studied using other data sources. To fill the research gap, this paper tries to develop a machine learning classifier to explore the characteristic space-time travel patterns of different subgroups, specifically gender (male/female/others). 
